...
Future<void> addNewTripData(Trip trip, File? urlToImage) async {
  ...
  try {
    ...
    if (trip.ispublic) {
      AdminCloudFunction().logError('Error saving new public trip:  $e');
    } else {
      AdminCloudFunction().logError('Error saving new private trip:  $e');
    }
  }
  try {
    ...
    AdminCloudFunction().logError('Error saving member data to new trip:  $e');
  }
  ...
  if (urlToImage != null) {
    try {
      ...
      AdminCloudFunction().logError('Error saving trip image:  '
          '$e');
    }
  }
}
  ...
  if (!trip.ispublic) {
    try {
      ...
      try {
        ...
        AdminCloudFunction().logError('Error deleting private trip after '
            'converting to public trip:  $e');
      }
      for (final String member in trip.accessUsers) {
        TripCloudFunctions().addMember(trip.documentId, member);
      }
    } catch (e) {
      AdminCloudFunction().logError('Error converting to public trip: ' '$e');
    }
  } else {
    try {
      ...
      for (final String member in trip.accessUsers) {
        TripCloudFunctions().addPrivateMember(trip.documentId, member);
      }
    } catch (e) {
      AdminCloudFunction().logError('Error converting to private trip:  $e');
    }
  }
}
...
Future<void> editTripData(
  ...
  try {
    ...
    AdminCloudFunction().logError('Error editing public trip:  $e');
  }
  try {
...
Future<Trip> getTrip(String documentID) async {
  ...
  try {
    ...
    AdminCloudFunction().logError('Error retrieving single trip by docID:  $e');
    return Trip.mock();
  }
}
...
Future<Trip> getPrivateTrip(String documentID) async {
  ...
  try {
    ...
    AdminCloudFunction().logError('Error retrieving single private trip:  $e');
    return Trip.mock();
  }
}
