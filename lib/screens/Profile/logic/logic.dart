...
Future<void> updateUserData(String email, String uid) async {
  try {
    ...
    AdminCloudFunction().logError('Error updating user data:  $e');
  }
}
...
Future<void> saveDeviceToken() async {
  try {
    ...
    AdminCloudFunction().logError('Error saving token:  $e');
  }
}
...
Future<bool> checkUserHasProfile() async {
  final DocumentReference<Object?> ref =
      userCollection.doc(userService.currentUserID);
...
Future<void> updateUserPublicProfileData({
  ...
  if (displayName?.isEmpty ?? true) {
    displayName = 'User${uid.substring(uid.length - 5)}';
  }
  try {
    ...
    AdminCloudFunction().logError('Error creating public profile:  $e');
  }
}
...
Future<void> editPublicProfileData(
  ...
  try {
    ...
    AdminCloudFunction().logError('Error editing public profile:  $e');
    if (kDebugMode) {
  ...
  if (urlToImage != null) {
    ...
    try {
      ...
      final Reference storageReference = FirebaseStorage.instance
          .ref()
          .child('users/${userService.currentUserID}');
...
Stream<List<UserPublicProfile>> retrieveFollowList(
  ...
  try {
    ...
    AdminCloudFunction().logError('Error retrieving follow list:  $e');
  }
}
...
List<Trip> _pastCrewTripListFromSnapshot(QuerySnapshot<Object?> snapshot) {
  try {
    ...
    AdminCloudFunction().logError('Error retrieving past trip list:  $e');
    return <Trip>[];
  }
}
...
Future<List<UserPublicProfile>> usersList() async {
  try {
    ...
    AdminCloudFunction().logError('Error retrieving all users: $e');
    return <UserPublicProfile>[];
  }
}
