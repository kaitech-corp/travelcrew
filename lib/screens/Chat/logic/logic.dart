...
Future<void> addNewChatMessage(String tripDocID, String displayName,
  ...
  try {
    ...
    AdminCloudFunction().logError('Error writing new chat:  $e');
    if (kDebugMode) {
...
Future<void> clearChatNotifications(String tripDocID) async {
  try {
    ...
    AdminCloudFunction().logError('Error clearing chat notifications:  $e');
  }
}
...
Stream<List<ChatModel>>? get chatListNotification {
  ...
  try {
    ...
    AdminCloudFunction().logError('Error retrieving chat list notifications:  $e');
    return null;
  }
}
