...
class AllUserRepository extends GenericBlocRepository<UserPublicProfile> {
  ...
  Stream<List<UserPublicProfile>> data() {
    ...
    List<UserPublicProfile> userListFromSnapshot(
        QuerySnapshot<Object> snapshot) {
      try {
        List<UserPublicProfile> userList =
        ...
        print(e);
        AdminCloudFunction()
            .logError('Error retrieving stream of all users: $e');
        return <UserPublicProfile>[];
      }
    }

    // get all users
    return userPublicProfileCollection.snapshots().map(userListFromSnapshot);
  }
}
