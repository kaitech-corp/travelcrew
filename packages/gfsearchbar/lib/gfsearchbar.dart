...
typedef OnSubmitted<T> = void Function(T item);
...
class GFSearchBar<T> extends StatefulWidget {
  ...
  final Future<List<T>> searchList;
  ...
  /// defines what to do once text is submitted
...
class MySingleChoiceSearchState<T> extends State<GFSearchBar<T?>> {
  ...
  Future<void> init() async {
    ...
    isFocused = false;
    _list = List<T>.from(await widget.searchList);
    ...
    textController.addListener(() async {
      ...
      if (text.trim().isNotEmpty) {
        ...
        widget.searchQueryBuilder(text, await widget.searchList);
